...
/**
 * Created by Shivangi on 8/17/2017.
 *
...
public class FindDupsInFile {
    ...
    public void parse(String[] input){
        ...
        for(String s : input){
            ...
            for(int i=1; i<directory.length; i++){
                String[] folder = directory[i].split("\\(");
                String file = folder[1].replace(")", "");
                List<String> list = map.getOrDefault(file, new ArrayList<String>());
                list.add(directory[0]+"/"+folder[0]);
                map.put(file, list);
            }
        }
    ...
    public static void main(String args[]){
        FindDupsInFile fd = new FindDupsInFile();
